...
def plot_posteriors(mnest, prefix, multinest_results, parameters, mds_orig):
    ...
    # Helper: format labels as in __multinest.plot_posteriors and add deriveds for corner
    def _format_labels_for_corner(base_labels, param):
        mol2base = {
            'H2O': 'H$_2$O', 'NH3': 'NH$_3$', 'CH4': 'CH$_4$', 'H2S': 'H$_2$S',
            'SO2': 'SO$_2$', 'CO2': 'CO$_2$', 'CO': 'CO', 'O2': 'O$_2$',
            'O3': 'O$_3$', 'N2O': 'N$_2$O', 'N2': 'N$_2$', 'H2': 'H$_2$'
        }
        ...
        for lbl in base_labels:
            # Surface pressure
            if lbl == 'P$_0$':
                pretty.append('Log(P$_0$ [Pa])')
                continue
            # Clouds
            if lbl.startswith('Log(P$_{top, H_2O}$'):
                pretty.append('Log(P$_{top, H_2O}$ [Pa])')
                continue
            if lbl.startswith('Log(D$_{H_2O}$'):
                pretty.append('Log(D$_{H_2O}$ [Pa])')
                continue
            if lbl.startswith('Log(CR$_{H_2O}$'):
                pretty.append('Log(CR$_{H_2O}$)')
                continue
            if lbl.startswith('Log(P$_{top, NH_3}$'):
                pretty.append('Log(P$_{top, NH_3}$ [Pa])')
                continue
            if lbl.startswith('Log(D$_{NH_3}$'):
                pretty.append('Log(D$_{NH_3}$ [Pa])')
                continue
            if lbl.startswith('Log(CR$_{NH_3}$'):
                pretty.append('Log(CR$_{NH_3}$)')
                continue
            # Gases -> use formatted_labels (Log(species))
            found_gas = False
            for mol, blbl in mol2base.items():
                if lbl == blbl and mol in param.get('supported_molecules', []):
                    pretty.append(param['formatted_labels'][mol])
                    found_gas = True
                    break
            if found_gas:
                continue
            # Surface albedo and cutoffs
            if lbl == '$a_{surf}$' or lbl.startswith('$a_{surf,'):
                pretty.append(lbl)
                continue
            if lbl.startswith('$\\lambda_{surf'):
                # add microns unit
                pretty.append(lbl.replace('$\\lambda', '$\\lambda') + ' [$\\mu$m]')
                continue
            # Temperature
            if lbl == 'T$_p$':
                pretty.append('T$_p$')
                continue
            # Cloud fraction (stored and sampled in log space)
            if lbl == 'cld frac':
                pretty.append('Log(cld frac)')
                continue
            # Gravity
            if lbl == 'Log(g)':
                pretty.append('Log(g [m/s$^2$])')
                continue
            # Mass / Radius with units
            if lbl == 'M$_p$':
                if param.get('rocky'):
                    pretty.append('M$_p$ [M$_\\oplus$]')
                else:
                    pretty.append('M$_p$ [M$_J$]')
                continue
            if lbl == 'R$_p$':
                if param.get('rocky'):
                    pretty.append('R$_p$ [R$_\\oplus$]')
                else:
                    pretty.append('R$_p$ [R$_{Jup}$]')
                continue
            # Particle size
            if lbl == 'P$_{size}$':
                if param.get('p_size_type') == 'factor':
                    pretty.append('Log(P$_{size, fctr})$')
                else:
                    pretty.append('Log(P$_{size}$ [$\\mu$m])')
                continue
            # Phase(s)
            if lbl.startswith('$\\phi'):
                pretty.append(lbl)
                continue
            # Fallback
            pretty.append(lbl)

        # Add derived labels at the end (only if gases are present)
        has_gases = len(param.get('fit_molecules', [])) > 0
        # P0 derived when using partial-pressure space and not explicitly fit
        if has_gases and (param.get('gas_par_space') == 'partial_pressure') and ('P$_0$' not in base_labels):
            pretty.append('Log(P$_0$ [Pa]) (derived)')
        # Gas fill derived
        if has_gases and (param.get('gas_fill') is not None):
            if param.get('rocky'):
                pretty.append(param['formatted_labels'][param['gas_fill']] + ' (derived)')
            else:
                pretty.append('Log(H$_2$ + He) (derived)')
        # Mean molecular mass derived
        if has_gases:
            pretty.append('$\\mu$ (derived)')
    ...
    def _compute_derived_columns(samples, base_labels, param):
        """Compute derived columns (P0, gas_fill, mu) for each sample.
        Returns list of arrays to append in order matching _format_labels_for_corner extras.
        """
        # Map molecule labels to indices in samples
        mol2base = {
            'H2O': 'H$_2$O', 'NH3': 'NH$_3$', 'CH4': 'CH$_4$', 'H2S': 'H$_2$S',
            'SO2': 'SO$_2$', 'CO2': 'CO$_2$', 'CO': 'CO', 'O2': 'O$_2$',
            'O3': 'O$_3$', 'N2O': 'N$_2$O', 'N2': 'N$_2$', 'H2': 'H$_2$'
        ...
        mol_idxs = []
        mol_list = []
        for mol in param.get('fit_molecules', []):
            blbl = mol2base.get(mol)
            if blbl is None:
                continue
            try:
                mol_idxs.append(base_labels.index(blbl))
                mol_list.append(mol)
            except ValueError:
                pass
        ...
        # Derived P0 (partial pressure)
        if (param.get('gas_par_space') == 'partial_pressure') and ('P$_0$' not in base_labels) and len(mol_idxs) > 0:
            pp = 10.0 ** (samples[:, mol_idxs])
            p0 = np.log10(np.sum(pp, axis=1))
            derived_cols.append(p0)

        # Compute vmr for gases + gas_fill
        vmr = None
        gas_fill_log = None
        if len(mol_idxs) > 0:
            space = param.get('gas_par_space')
            if space in ('volume_mixing_ratio', 'vmr'):
                vmr_m = 10.0 ** (samples[:, mol_idxs])
                vmr_fill = 1.0 - np.clip(np.sum(vmr_m, axis=1), 0.0, 1.0)
                vmr_fill = np.maximum(vmr_fill, 1e-30)
                vmr = (vmr_m, vmr_fill)
            elif space == 'partial_pressure':
                pp = 10.0 ** (samples[:, mol_idxs])
                tot = np.sum(pp, axis=1)
                tot = np.maximum(tot, 1e-300)
                vmr_m = pp / tot[:, None]
                vmr_fill = 1.0 - np.clip(np.sum(vmr_m, axis=1), 0.0, 1.0)
                vmr_fill = np.maximum(vmr_fill, 1e-30)
                vmr = (vmr_m, vmr_fill)
            elif space in ('centered_log_ratio', 'clr'):
                z = samples[:, mol_idxs]
                e = np.exp(z)
                last = np.exp(-np.sum(z, axis=1))[:, None]
                comp = np.concatenate([e, last], axis=1)
            ...
            if vmr is not None:
                vmr_m, vmr_fill = vmr
                if param.get('gas_fill') is not None:
                    gas_fill_log = np.log10(np.maximum(vmr_fill, 1e-30))

                # Mean molecular mass (mu)
                mm = param['mm']
                # Sum contributions from fitted molecules
                mu = np.zeros(n)
                for k, mol in enumerate(mol_list):
                    mu += vmr_m[:, k] * mm[mol]
                if param.get('gas_fill') is not None:
                    if param.get('rocky'):
                        mu += vmr_fill * mm[param['gas_fill']]
                    else:
                        # split H2 + He according to ratio
                        ratio = float(param.get('H2_He_ratio', 0.85))
                        mu += vmr_fill * (ratio * mm['H2'] + (1.0 - ratio) * mm['He'])

                if gas_fill_log is not None:
                    derived_cols.append(gas_fill_log)
                derived_cols.append(mu)
        # If no gases, no derived columns are added
